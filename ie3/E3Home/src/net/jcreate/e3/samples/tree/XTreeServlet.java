...
public class XTreeServlet extends DispatchServlet{
	...
	public void showNavTree(final HttpServletRequest pRequest, 
		...
		WebTreeNode tagNode = new WebTreeNode("taglib树", "tag");		
		rootNode.addNode(tagNode);
		tagNode.setAction("javascript:openURL('" +
	              RequestUtil.getUrl("/servlet/xtreeServlet?_actionType=showTagTree" +
	              "')"				            		  
			, pRequest));
		WebTreeNode adNode = new WebTreeNode("赞助商", "ad");		
		rootNode.addNode(adNode);
		adNode.setAction("javascript:openURL('" +
	              RequestUtil.getUrl("/e3/ad/Ad.jsp" +
	              "')"				            		  
			, pRequest));
		WebTreeNode homeNode = new WebTreeNode("返回主页", "home");		
		rootNode.addNode(homeNode);
		homeNode.setAction("javascript:goHome('" +
	              RequestUtil.getUrl("/Index.jsp" +
	              "')"				            		  
			, pRequest));
...
pRequest.getRequestDispatcher("/e3/samples/tree/CheckTree.jsp").forward(pRequest,pResponse);
	...
	public void	showExtLoadTree(final HttpServletRequest pRequest, 
		...
		public void	showTagTree(final HttpServletRequest pRequest, 
	            final HttpServletResponse pResponse) throws Exception{
		TreeService treeService =  TreeBeanFactory.getTreeService();		
		List orgs =  treeService.getOrgs();
		pRequest.setAttribute("orgs", orgs);
		pRequest.getRequestDispatcher("/e3/samples/tree/TagTree.jsp").forward(pRequest,pResponse);		
	}
